...
## 2024-05-18 - Lua 5.1 Regex Alternation Failure and O(1) Lookup
**Learning:** Lua 5.1 `string.match` does not support standard regex alternation (like `|`). Patterns like `^(the|die)$` silently fail and do not match, causing bugs and unexpected behavior. Additionally, doing repetitive regex evaluations on frame updates can cause UI stuttering.
**Action:** Use O(1) table lookups (hash maps) instead of attempting alternation for multi-string exact matching. This ensures correctness in Lua 5.1 and provides better performance, especially during frequent operations like LFG list polling.
